...
module LogplexFace 
  ...
  def set_weight(ip, weight)
    ...
    `echo "set weight logplexdirect/#{ip} #{weight}" | socat unix-connect:/tmp/haproxy.sock stdio`
    `echo "set weight logplexviasyslog/#{ip} #{weight}" | socat unix-connect:/tmp/haproxy.sock stdio`
    log("update http weight", `echo "get weight logplexapi/#{ip}" | socat unix-connect:/tmp/haproxy.sock stdio`.strip)
    log("update tcp weight", `echo "get weight logplexdirect/#{ip}" | socat unix-connect:/tmp/haproxy.sock stdio`.strip)
    log("update syslog weight", `echo "get weight logplexviasyslog/#{ip}" | socat unix-connect:/tmp/haproxy.sock stdio`.strip)
  end
